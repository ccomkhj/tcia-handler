...
class PreprocessingPipeline:
    ...
    def run_command(self, cmd: list, description: str) -> bool:
        ...
        try:
            result = subprocess.run(
                cmd, cwd=str(self.mri_root), check=True, capture_output=True, text=True
            )
    ...
    def step_generate_ignore_list(self) -> bool:
        """Step 1.5: Generate ignore list from feedback."""
        logger.info("\n" + "=" * 80)
        logger.info("STEP 1.5: Generate Ignore List")
        logger.info("=" * 80)

        cmd = ["python", str(self.preprocessing_dir / "generate_ignore_list.py")]
        return self.run_command(cmd, "Ignore list generation")
    ...
    def step_2_merge_datasets(self) -> bool:
        ...
        # Check if already done (Force re-run if ignore list exists? No, keep standard check)
        # But if ignore list is new, we might want to re-merge.
        # For now, rely on user to clean up if they want re-run, or remove the check.
        # But I'll stick to the existing pattern.
    ...
    def step_4_convert_dicom_to_png(self, class_num: int = None) -> bool:
        ...
        if class_num:
            ...
            cmd = [
                "python",
                str(self.preprocessing_dir / "dicom_converter.py"),
                "--class",
                str(class_num),
            ]
    ...
    def step_5_process_overlays(self) -> bool:
        ...
        cmd = ["python", str(self.preprocessing_dir / "process_overlay_to_masks.py")]
    ...
    def run_all(self, skip_manual_steps: bool = True) -> bool:
        ...
        steps = [
            ...
            ("Generate Ignore List", self.step_generate_ignore_list),
            ("Merge Datasets", self.step_2_merge_datasets),
        ]

        if not skip_manual_steps:
            steps.append(
                ("Generate TCIA Manifests", self.step_3_generate_tcia_manifests)
            )
...
def main():
    ...
    if args.all:
        ...
        step_map = {
            ...
            "generate_ignore_list": pipeline.step_generate_ignore_list,
            "merge_datasets": pipeline.step_2_merge_datasets,
            "generate_tcia": pipeline.step_3_generate_tcia_manifests,
            "dicom_to_png": lambda: pipeline.step_4_convert_dicom_to_png(
                args.class_num
            ),
